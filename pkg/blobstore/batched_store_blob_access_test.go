...
func TestBatchedStoreBlobAccessSuccess(t *testing.T) {
	...
	baseBlobAccess.EXPECT().FindMissing(
		ctx,
		digest.NewSetBuilder(0).Add(digestHello).Add(digestEmpty).Build()).Return(
		digest.NewSetBuilder(0).Add(digestHello).Build(), nil)
	...
	baseBlobAccess.EXPECT().FindMissing(
		ctx,
		digest.NewSetBuilder(0).Add(digestGoodbye).Build()).Return(
		digest.NewSetBuilder(0).Add(digestGoodbye).Build(), nil)
...
func TestBatchedStoreBlobAccessFailure(t *testing.T) {
	...
	require.NoError(t, blobAccess.Put(ctx, digestGoodbye, buffer.NewValidatedBufferFromByteSlice([]byte("Goodbye"))))
	baseBlobAccess.EXPECT().FindMissing(ctx, digest.NewSetBuilder(0).Add(digestGoodbye).Build()).Return(digest.EmptySet, nil)
	require.NoError(t, flush(ctx))
}
